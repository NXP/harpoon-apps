...
static uint32_t os_counter_tpm_us_to_ticks(os_counter_t *dev, uint64_t period_us)
{
	return period_us * SOURCE_CLOCK_FREQ_MHZ;
}

static uint64_t os_counter_tpm_ticks_to_ns(const os_counter_t *dev, uint32_t ticks)
{
...
static const struct os_counter_ops tpm_counter_ops = {
	...
	.os_counter_us_to_ticks = os_counter_tpm_us_to_ticks,
	.os_counter_ticks_to_ns = os_counter_tpm_ticks_to_ns,
